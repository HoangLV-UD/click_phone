...
import com.example.world_phone.dto.respone.voucher.VoucherUseByOrderResponse;
...
import static com.example.world_phone.type.CustomerStatus.FAILED;
import static com.example.world_phone.type.CustomerStatus.SUCCESS;
...
public class VoucherApi {
    ...
    public ResponseEntity<String> createVoucher(@RequestBody @Validated VoucherRequest voucherRequest, Errors errors) {
        ...
        try {
            ...
            if (Long.parseLong(voucherRequest.getVoucherDiscount()) <= 0) {
                throw new WorldPhoneExp(ConstansErrorCode.ERROR_MIN_MONEY, "giảm giá", "1đ");
            }
            ...
            throw new WorldPhoneExp(ConstansErrorCode.ERROR_FORMAT_NUMBER);
        } catch (Exception e) {
            e.printStackTrace();
        }
        throw new WorldPhoneExp(ConstansErrorCode.ERROR_SAVE_FAILED);
    }
    ...
    public ResponseEntity<String> updateVoucher(@RequestBody @Validated VoucherRequest voucherRequest, Errors errors) {
        ...
        if (Objects.isNull(voucherResponse) || Objects.isNull(voucherResponse.getVoucherId())) {
            throw new WorldPhoneExp(ConstansErrorCode.ERROR_NOT_EXISTS, "Mã giảm giả");
        }
...
//            }
    ...
    public ResponseEntity<VoucherResponse> getById(@PathVariable("id") String id) {
        ...
        if (Objects.isNull(voucherResponse) || voucherResponse.getVoucherStartDate().compareTo(new Date()) < 0) {
            // voucher không đúng
            throw new WorldPhoneExp(ConstansErrorCode.VOUCHER_NOT_EXISTS);
        }
